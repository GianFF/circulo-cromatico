...
describe('ChromaticCircle', () => {
  describe('mixColors', () => {
    ...
    it('should throw an error for invalid filter combinations', () => {
      const circulo = new ChromaticCircle(Color.NOT_A_COLOR);

      expect(() => circulo.mixColors(Color.RED)).toThrow('No color mixer found that can handle filter: not-a-color');
    });
  });
});
